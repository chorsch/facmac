...
def run_sequential(args, logger):
    ...
    # Setup multiagent controllers here
    #ADJUSTED FOR PARTITIONS
    #one controller per partition - controllers decide actions
    mac = []
    for partition in partitions:
        mac.append(mac_REGISTRY[args.mac](buffer.scheme, groups, args))

    # Give runner the scheme
    runner.setup(scheme=scheme, groups=groups, preprocess=preprocess, macs=macs)

    # Learner
    #ADJUSTED FOR PARTITIONS - one learner per partition
    learners = []
    for partition in partitions:
        learners = [le_REGISTRY[args.learner](mac, buffer.scheme, logger, args) for mac in macs]
    ...
    while runner.t_env <= args.t_max:
        ...
        if getattr(args, "runner_scope", "episodic") == "episodic":
            #learners not used
            episode_batch = runner.run(test_mode=False, learners=learners)
            ...
            if buffer.can_sample(args.batch_size) and (buffer.episodes_in_buffer > getattr(args, "buffer_warmup", 0)):
                ...
                # NEED TO SPLIT UP OBSERVATIONS AND REWARDS INTO PARTITIONS HERE
